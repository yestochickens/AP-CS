...
{
    BYTE hex[] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F, 0x10, 0x11, 0x12,
    ...
    for (int i = 0; i < height; i++)
    {
        for (int j = 0; j < width; j++)
        {
            ...
            {
                image[i][j].rgbtGreen = hex[rand() % 255];
                image[i][j].rgbtRed = hex[rand() % 255];
                image[i][j].rgbtBlue = hex[rand() % 255];
            }
        }
    }
}
